import {
  ...
  interface SearchSongsTableProps {
    searchQuery: string; // Nhận query để gọi lại tìm kiếm nếu cần
  }
  
  const SearchSongsTable = ({ searchQuery }: SearchSongsTableProps) => {
    const {
      searchResults,
      isLoading,
      error,
    ...
    const sortedSongs = [...searchResults].sort((a, b) => {
    ...
    const debouncedFetchSongs = debounce((page: number, limit: number) => {
      fetchSearchSongs(searchQuery, page, limit);
    }, 300);
  
    const handlePageChange = (newPage: number) => {
      if (newPage >= 1 && newPage <= searchTotalPages) {
        debouncedFetchSongs(newPage, searchLimit);
      }
    };
      ...
      <div className="bg-zinc-900 rounded-xl shadow-lg border border-zinc-800/50 overflow-hidden overflow-y-hidden">
        ...
        >
          <div className="text-zinc-400 text-sm hidden md:block">
            Showing {searchResults.length} of {searchTotal} songs
          </div>
          {searchTotalPages > 1 && (
            <Pagination>
              <PaginationContent className="bg-zinc-800/50 rounded-full p-2 shadow-md">
                  ...
                  .map((page) => (
                    <PaginationItem key={page}>
                      <PaginationLink
                        ...
                        className={`rounded-full hover:bg-zinc-700 text-white border-zinc-700 transition-colors ${
                          searchPage === page ? "bg-emerald-600 text-white" : ""
                        }`}
                      />
                    </PaginationItem>
                  ))}
                ...
                {searchPage < searchTotalPages - 1 && (
                  <PaginationItem>
                    <PaginationLink
                      onClick={() => handlePageChange(searchTotalPages)}
                      className="rounded-full hover:bg-zinc-700 text-white border-zinc-700 transition-colors"
                    >
                      {searchTotalPages}
                    </PaginationLink>

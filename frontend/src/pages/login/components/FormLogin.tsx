...
export function LoginForm({ className, ...props }: React.ComponentProps<"div">) {
  ...
  const signInWithEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === "complete") {
        window.location.href = "/auth-callback"; // Điều hướng sau khi đăng nhập thành công
      }
    } catch (err: any) {
  ...
  return (
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      <Card className="overflow-hidden border border-zinc-800/50 bg-zinc-900/95 shadow-xl">
        <CardContent className="grid p-0 md:grid-cols-2">
          <form onSubmit={signInWithEmail} className="p-6 md:p-8">

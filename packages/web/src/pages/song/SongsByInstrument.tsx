...
import { Heart, Music, Play, ChevronDown, ChevronUp } from "lucide-react";
...
const SongsByInstrument = () => {
  ...
  const INITIAL_DISPLAY_COUNT = 18; // Giới hạn số bài hát hiển thị ban đầu

  useEffect(() => {
    if (instrumentId) {
      fetchSongByInstruments(instrumentId, 1, 200);
      if (!instruments.length) fetchInstruments();
    }
    if (info?._id) {
  ...
  useEffect(() => {
    const status = songByInstruments.reduce((acc, song) => {
      acc[song._id] = favorites.some((fav) => fav._id === song._id);
      return acc;
    }, {} as { [key: string]: boolean });
    setFavoriteStatus(status);
  }, [favorites, songByInstruments]);

  const currentInstrument = instruments.find((i) => i._id === instrumentId);
          ...
          <BreadcrumbList className="text-zinc-400">
            ...
            <BreadcrumbItem>
              <BreadcrumbLink href="#" className="text-sky-500 font-semibold">
                <Link to={`/instruments/${currentInstrument?._id}`}>
                  {currentInstrument?.name || "Instrument"}
                </Link>
              </BreadcrumbLink>
            </BreadcrumbItem>
          </BreadcrumbList>
        ...
        <div className="relative min-h-full bg-gradient-to-b from-zinc-900 via-zinc-950 to-black">
          ...
          >
            ...
            >
              ...
              >
                {currentInstrument?.name || "Unknown Instrument"}
              </motion.h1>
              <motion.p
                variants={itemVariants}
              ...
              >
                {currentInstrument?.description ||
                  "Explore the best tracks featuring this instrument."}
              </motion.p>
              <motion.div
                variants={itemVariants}
              ...
              >
                <Button
                  ...
                  onClick={() =>
                    setCurrentSong(songByInstruments[0], info?.clerkId || "")
                  }
                  disabled={!songByInstruments.length}
                >
                  <Play className="h-5 w-5 mr-2 fill-black" /> Play All
                </Button>
                <span className="text-zinc-300">
                  {songByInstruments.length} songs
                </span>
              </motion.div>
            </motion.div>
          </motion.section>
          ...
          >
            ...
            >
              {songByInstruments.slice(0, INITIAL_DISPLAY_COUNT).map((song) => (
            ...
            {/* Collapsible Section */}
            {songByInstruments.length > INITIAL_DISPLAY_COUNT && (
              <Collapsible
                open={isOpen}
            ...
            {songByInstruments.length === 0 && (
              ...
              >
                No songs found for this instrument.
              </motion.p>
            )}
          </motion.div>
        </div>
...
export default SongsByInstrument;

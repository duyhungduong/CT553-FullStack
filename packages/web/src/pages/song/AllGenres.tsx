...
const AllGenres = () => {
  const { genres, fetchGenres, isLoading } = useMusicStore();
  ...
  const INITIAL_DISPLAY_COUNT = 12; // Giới hạn số thể loại hiển thị ban đầu
  ...
  // Lọc danh sách thể loại dựa trên từ khóa tìm kiếm
        ...
        <div className="relative min-h-full bg-gradient-to-b from-zinc-900 via-zinc-950 to-black">
          ...
          >
            ...
            <motion.div variants={containerVariants} className="relative z-10 text-center">
              ...
              >
                Discover a world of music with {genres.length} unique genres to explore.
              </motion.p>
            </motion.div>
          </motion.section>
          ...
          >
            <div className="flex justify-between items-center mb-8">
              ...
              >
                <Music className="h-6 w-6 text-green-500" /> All Genres
              </motion.h2>
            ...
            >
              {filteredGenres.slice(0, INITIAL_DISPLAY_COUNT).map((genre) => (
                <motion.div key={genre._id} variants={itemVariants}>
                  <Card className="group rounded-xl bg-zinc-800/50 border-zinc-700 h-full shadow-md hover:shadow-xl hover:bg-zinc-700/90 transition-all duration-300 overflow-hidden">
                    <CardContent className="p-4 relative">
                      <div className="relative mb-3">
                        <div className="aspect-square rounded-lg overflow-hidden shadow-md">
                          <img
                            src={genre.imageUrl || "https://via.placeholder.com/300x300"}
                            alt={genre.name}
            ...
            {/* Collapsible Section */}
            {filteredGenres.length > INITIAL_DISPLAY_COUNT && (
                  ...
                  {isOpen && (
                    <CollapsibleContent asChild>
                      ...
                      >
                        ...
                        >
                          {filteredGenres.slice(INITIAL_DISPLAY_COUNT).map((genre) => (
                            <motion.div key={genre._id} variants={itemVariants}>
                                ...
                                <CardContent className="p-4 relative">
                                  <div className="relative mb-3">
                                    <div className="aspect-square rounded-lg overflow-hidden shadow-md">
                                      <img
                                        src={
                                          genre.imageUrl || "https://via.placeholder.com/300x300"
                                        }
                                        alt={genre.name}
                                        className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105"
                                      />
                    ...
                    >
                      <span className="relative z-10 flex items-center gap-2">
                        {isOpen ? (
                          ...
                          <>
                            Show More ({filteredGenres.length - INITIAL_DISPLAY_COUNT})

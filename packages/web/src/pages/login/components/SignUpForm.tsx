...
export function SignUpForm({ className, ...props }: React.ComponentProps<"div">) {
  ...
  const signUpWithEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === "complete") {
        window.location.href = "/auth-callback"; // Điều hướng sau khi đăng ký thành công
      } else if (result.status === "missing_requirements") {
        // Nếu cần xác minh email
        await signUp.prepareEmailAddressVerification();
        setError("Please verify your email to complete sign-up.");
      }
    } catch (err: any) {
  ...
  return (
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      <Card className="overflow-hidden overflow-y-hidden border border-zinc-800/50 bg-zinc-900/95 shadow-xl">
        <CardContent className="grid p-0 md:grid-cols-2">
          <form onSubmit={signUpWithEmail} className="p-6 md:p-8">
            <div className="flex flex-col gap-6">
              <div className="flex flex-col items-center text-center">
                <h1 className="text-2xl font-bold font-outfit text-white">Join Melodia</h1>
      ...
      <div className="text-balance text-center text-xs text-zinc-400 [&_a]:underline [&_a]:underline-offset-4 hover:[&_a]:text-emerald-400">
